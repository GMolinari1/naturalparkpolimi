...
function load()
{
  ...
  $.getJSON('../php/territorioCategoria.php?query=' +$_GET["var"], function(data) 
  {
    var newhtml = '';
    $.each(data, function(key, val) 
    {
      if (key>= position && key < position+delta) 
      {
        ...
        newhtml += '<a href="http://naturalparkpolimi.altervista.org/src/territorioScelto.html?var='+ val.id_ter+'"><img src="'+val.fotopath+'"></img></a>';
        newhtml += '</figure>';
        newhtml += '<figure class="col-sm-8">';
        newhtml += '<a href="http://naturalparkpolimi.altervista.org/src/territorioScelto.html?var='+ val.id_ter+'"><h4>' + val.nome +'</h4></a>';
